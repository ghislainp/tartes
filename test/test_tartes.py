...
def test_one_layer_diffuse(snowpack_1layer_ssa20):
    ...
    alb = albedo(wavelength, **snowpack_1layer_ssa20.as_dict(), B0=1.6, g0=0.86)
...
def test_w1995(snowpack_1layer_ssa20):
    ...
    alb = albedo(wavelength, **snowpack_1layer_ssa20.as_dict(), refrac_index="w1995", B0=1.6, g0=0.86)
    print(alb)
...
def test_one_layer_direct(snowpack_1layer_ssa20):
    ...
    alb = albedo(wavelength, **snowpack_1layer_ssa20.as_dict(), dir_frac=1, sza=30, B0=1.6, g0=0.86)
    print(alb)
...
def test_multilayer(snowpack_multilayer):
    ...
    alb = albedo(wavelength, SSA=snowpack_multilayer.SSA,
                 density=snowpack_multilayer.density,
                 thickness=snowpack_multilayer.thickness, B0=1.6, g0=0.86)
...
def test_diffus(snowpack_multilayer):
    ...
    alb_dir48 = albedo(wavelength, **snowpack_multilayer.as_dict(), sza=sza_eff, dir_frac=1)
    alb_diff = albedo(wavelength, **snowpack_multilayer.as_dict(), dir_frac=0)

    assert np.allclose(alb_dir48, alb_diff)

    alb_halfdir48 = albedo(wavelength, **snowpack_multilayer.as_dict(), sza=sza_eff, dir_frac=0.5)
...
def test_impurities(snowpack_1layer_ssa20):
    ...
    alb = albedo(wavelength, **snowpack_1layer_ssa20.as_dict(), impurities=100e-9, B0=1.6, g0=0.86)
...
def test_flux_actinic(snowpack_1layer_ssa20):

    wavelength = 450e-9

    z = [0.1, 0.2]

    act = actinic_profile(wavelength, z, **snowpack_1layer_ssa20.as_dict(), dir_frac=1, sza=30, B0=1.6, g0=0.86)
...
def test_consistency_irradiance(snowpack_1layer_ssa20):
    ...
    sp['thickness'] = thickness
    alb = albedo(wls, **sp, dir_frac=1, sza=sza)
    irr_dn, irr_up = irradiance_profiles(wls, z=[0], **sp, dir_frac=1, sza=sza)
